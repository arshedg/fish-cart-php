...
/* --------------------------------------------------------------- */	
	...
	$(document).on('click','.product-trigger', function(e) {
		e.preventDefault();
		var product_top_pos = $(window).scrollTop();
		if(product_top_pos + pr.height() > $(document).height()) {
			$.scrollTo(0, 800, { axis:'y' });
			product_top_pos = 0;
		}
		pr.css('top', product_top_pos + 'px');
...
/* --------------------------------------------------------------- */	
	...
	function productPieceBtns(el) {
		...
		var price = el.find('input[type=hidden][name=price]');
		el.find('.product-pieces-up').on('click', function(e) {
			value = parseInt(input.val(), 10);
			if(!value || value < 0)
				value = 1;
			input.val(value+1);
			computeAmount(input.val(),price.val());
		});
